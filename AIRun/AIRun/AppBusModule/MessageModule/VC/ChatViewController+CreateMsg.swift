...
//            let msgID = V2TIMManager.sharedInstance().insertC2CMessage(toLocalStorage: imMsg, to: self.aiMID.stringValue, sender: APPManager.default.loginUserModel?.user?.uid.stringValue) {
//                
//            } fail: { code, errorStr in
//
//            }
            cellData.msgStatus = .Sending
//            cellData.msgID = msgID ?? ""
