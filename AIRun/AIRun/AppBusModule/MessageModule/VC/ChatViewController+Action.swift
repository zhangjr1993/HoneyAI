...
//                    for i in 0..<tempMsgs.count {
//                        visibleHeight += self.tableView(self.tableView, heightForRowAt: IndexPath(row: i, section: 0))
//                    }
//                    self.tableView.scrollRectToVisible(CGRect(x: 0, y: self.tableView.contentOffset.y + visibleHeight, width: UIScreen.screenWidth, height: self.tableView.height), animated: false)
//                    self.tableView.scrollToBottom(animated: false)
                }else {
                    if self.chatType != .privete, self.uiMsgs.count > 0 {                        
