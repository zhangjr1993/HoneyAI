...
extension HomeCommonListCell {
    func showDataModel(_ model: HomeCommonListModel, isFollow: Bool) {
        self.mid = model.mid
...
extension HomeCommonListCell {
    ...
    private func clickAttenAction() {
        self.attenBtn.addClickTime()
        APPManager.default.aiAttentionReq(mid: self.mid, isAtten: !self.attenBtn.isSelected) { [weak self] result in
            guard let `self` = self else { return }
            if result {
